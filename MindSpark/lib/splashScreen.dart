...
class _SplashState extends State<Splash> with SingleTickerProviderStateMixin {
                ...
                child: Row(
                  children: [
                    ...
                    JumpingDotsProgressIndicator(
                        color: Hexcolor("f3d266"), fontSize: 50),
                  ],
                )),
          ],
        ));
  ...
  Future<void> secondPostData(BuildContext context) async {
    Provider.of<ArticleModel>(context, listen: false)
        .setList(await fetchAricleData(context));
    Provider.of<PostModel>(context, listen: false)
        .setList(await fetchPostData(context));
    Provider.of<UserModel>(context, listen: false)
        .setUser(await getUserData(context));
    Provider.of<UserModel>(context, listen: false)
        .setExtraUser(await getExtraUserData(context));
  }
  ...
  Future<List<Post>> fetchPostData(BuildContext context) async {
    ...
    for (Post post in list) {
      ...
      var responseComment = await http.get(
          "https://mindsparkapi.herokuapp.com/api/v1/posts/comment/?post_id=${post.id}",
          headers: {
            "Authorization": "$token",
          });
  ...
  Future<List<Article>> fetchAricleData(BuildContext context) async {
    ...
    print("$token");
    var response = await http.get(
        "https://mindsparkapi.herokuapp.com/api/v1/articles/get/",
        headers: {"Authorization": "$token"});
    var responseBody = json.decode(response.body);
    print('responseBodyHome type is ${responseBody.runtimeType}');
    print('responseBodyHome length is ${responseBody.length}');
    ...
    for (Article article in list) {
      List<Comment> commentList = new List();
      var responseComment = await http.get(
          "https://mindsparkapi.herokuapp.com/api/v1/articles/comment/?article_id=${article.id}",
          headers: {
            "Authorization": "$token",
          });
      var responseBodyComment = json.decode(responseComment.body);
  ...
  Future<ExtraUser> getExtraUserData(BuildContext context) async {
    ...
    var response = await http.get(
        "https://mindsparkapi.herokuapp.com/api/v1/users/props?user_id=${userId}",
        headers: {"Authorization": "$token"});
    print("Made it past api call");
    var responseBody = json.decode(response.body);
